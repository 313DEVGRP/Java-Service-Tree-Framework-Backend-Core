...
public class JiraServerImpl extends TreeServiceImpl implements JiraServer{
	...
	public JiraServerEntity addJiraServer(JiraServerEntity jiraServerEntity) throws Exception {
		...
		if ( 등록결과 != null) {
			...
			if (jiraServerEntity.getC_jira_server_type().equals("온프라미스")) { // on-premise
				...
				서버_엔티티에_지라_이슈해결책_매핑(지라서버에_붙일_이슈해결책_리스트, 엔진통신기.지라_이슈해결책_가져오기(등록결과.getConnectId()));
				서버_엔티티에_지라_이슈상태_매핑(지라서버에_붙일_이슈상태_리스트, 엔진통신기.지라_이슈상태_가져오기(등록결과.getConnectId()));
			}
	...
	private void 서버_엔티티에_지라_이슈상태_매핑(Set<JiraIssueStatusEntity> 지라서버에_붙일_이슈상태_리스트, List<OnPremiseJiraStatusDTO> 엔진_지라이슈상태_목록) throws Exception {
		for ( OnPremiseJiraStatusDTO 이슈상태 : 엔진_지라이슈상태_목록) {
			if (지라_이슈상태_엔티티_검색결과(이슈상태) == null ) {
				JiraIssueStatusEntity 저장할_지라이슈상태 = 검색결과_미등록_이슈상태_엔티티_매핑(이슈상태);
				지라서버에_붙일_이슈상태_리스트.add(저장할_지라이슈상태);
			} else {
				logger.info("이미 존재하는 이슈상태 입니다. -> {}", 지라_이슈상태_엔티티_검색결과(이슈상태).getC_issue_status_id());
				지라서버에_붙일_이슈상태_리스트.add(지라_이슈상태_엔티티_검색결과(이슈상태));
			}
		}
	}

	private void 서버_엔티티에_지라_이슈해결책_매핑(Set<JiraIssueResolutionEntity> 지라서버에_붙일_이슈해결책_리스트, List<OnPremiseJiraResolutionDTO> 엔진_지라이슈해결책_목록) throws Exception {
		for( OnPremiseJiraResolutionDTO 이슈해결책 : 엔진_지라이슈해결책_목록) {
			if (지라_이슈해결책_엔티티_검색결과(이슈해결책) == null ) {
				지라서버에_붙일_이슈해결책_리스트.add(검색결과_미등록_이슈해결책_엔티티_매핑(이슈해결책));
			} else {
				logger.info("이미 존재하는 이슈해결책 입니다. -> " + 지라_이슈해결책_엔티티_검색결과(이슈해결책).getC_issue_resolution_id());
				지라서버에_붙일_이슈해결책_리스트.add(지라_이슈해결책_엔티티_검색결과(이슈해결책));
			}
		}
	}

	private void 서버_엔티티에_지라_이슈우선순위_매핑(Set<JiraIssuePriorityEntity> 지라서버에_붙일_이슈우선순위_리스트, List<OnPremiseJiraPriorityDTO> 엔진_지라이슈우선순위_목록) throws Exception {
		for ( OnPremiseJiraPriorityDTO 이슈우선순위 : 엔진_지라이슈우선순위_목록) {
			if (지라_이슈우선순위_엔티티_검색결과(이슈우선순위) == null ) {
				지라서버에_붙일_이슈우선순위_리스트.add(검색결과_미등록_이슈우선순위_엔티티_매핑(이슈우선순위));
			} else {
				logger.info("이미 존재하는 이슈우선순위 입니다. -> " + 지라_이슈우선순위_엔티티_검색결과(이슈우선순위).getC_issue_priority_id());
				지라서버에_붙일_이슈우선순위_리스트.add(지라_이슈우선순위_엔티티_검색결과(이슈우선순위));
			}
		}
	}

	private void 서버_엔티티에_지라_이슈타입_매핑(Set<JiraIssueTypeEntity> 지라서버에_붙일_이슈타입_리스트, List<OnPremiseJiraIssueTypeDto> 엔진_지라이슈타입목록) throws Exception {
		for ( OnPremiseJiraIssueTypeDto 이슈타입 : 엔진_지라이슈타입목록) {
			if( 지라_이슈타입_엔티티_검색결과(이슈타입) == null) {
				지라서버에_붙일_이슈타입_리스트.add(검색결과_미등록_이슈타입_엔티티_매핑(이슈타입));
			} else {
	...
	private void 서버_엔티티에_지라_프로젝트_매핑(Set<JiraProjectEntity> 지라서버에_붙일_프로젝트_리스트, List<OnPremiseJiraProjectDTO> 지라프로젝트목록) throws Exception {
		for ( OnPremiseJiraProjectDTO 지라프로젝트 : 지라프로젝트목록){
			if( 지라프로젝트_엔티티_검색결과(지라프로젝트) == null ){
				지라서버에_붙일_프로젝트_리스트.add(검색결과_미등록_프로젝트_엔티티_매핑(지라프로젝트));
			}else {
	...
	private void 서버_엔티티에_클라우드_지라_이슈상태_매핑(Set<JiraIssueStatusEntity> 지라서버에_붙일_이슈상태_리스트, List<Status> 클라우드_지라_상태_목록) throws Exception {
		for (Status 이슈상태 : 클라우드_지라_상태_목록) {
			if (지라_이슈상태_엔티티_검색결과(이슈상태) == null) {
				지라서버에_붙일_이슈상태_리스트.add(검색결과_미등록_이슈상태_엔티티_매핑(이슈상태));
			} else {
				logger.info("이미 존재하는 이슈상태 입니다. -> {}", 지라_이슈상태_엔티티_검색결과(이슈상태).getC_issue_status_id());
				지라서버에_붙일_이슈상태_리스트.add(지라_이슈상태_엔티티_검색결과(이슈상태));
			}
	...
	private void 서버_엔티티에_클라우드_지라_이슈해결책_매핑(Set<JiraIssueResolutionEntity> 지라서버에_붙일_이슈해결책_리스트, List<Resolution> 클라우드_지라_해결책_목록) throws Exception {
	...
	private void 서버_엔티티에_클라우드_지라_이슈우선순위_매핑(Set<JiraIssuePriorityEntity> 지라서버에_붙일_이슈우선순위_리스트, List<Priority> 클라우드_지라_우선순위_목록) throws Exception {
	...
	private void 서버_엔티티에_클라우드_지라_이슈타입_매핑(Set<JiraIssueTypeEntity> 지라서버에_붙일_이슈타입_리스트, List<CloudJiraIssueTypeDTO> 엔진_지라이슈타입목록) throws Exception {
		for ( CloudJiraIssueTypeDTO 이슈타입 : 엔진_지라이슈타입목록) {
			if( 지라_이슈타입_엔티티_검색결과(이슈타입) == null) { //기존 엔티티에 있는지 검색한 결과가 null 이면(등록안됨)
				지라서버에_붙일_이슈타입_리스트.add(검색결과_미등록_이슈타입_엔티티_매핑(이슈타입));
			} else {
				logger.info("이미 존재하는 프로젝트 입니다. -> " + 지라_이슈타입_엔티티_검색결과(이슈타입).getC_issue_type_id());
				지라서버에_붙일_이슈타입_리스트.add(지라_이슈타입_엔티티_검색결과(이슈타입));
			}
		}
	}

	private void 서버_엔티티에_클라우드_지라_프로젝트_매핑(Set<JiraProjectEntity> 지라서버에_붙일_프로젝트_리스트, List<CloudJiraProjectDTO> 클라우드_지라_프로젝트목록) throws Exception {
		for ( CloudJiraProjectDTO 지라프로젝트 : 클라우드_지라_프로젝트목록){
			if( 지라프로젝트_엔티티_검색결과(지라프로젝트) == null ){ // 아직 붙지 않은(추가되지 않은) 프로젝트 존재
				지라서버에_붙일_프로젝트_리스트.add(검색결과_미등록_프로젝트_엔티티_매핑(지라프로젝트));
			}else {
				logger.info("이미 존재하는 프로젝트 입니다. -> " + 지라프로젝트_엔티티_검색결과(지라프로젝트).getC_jira_key());
				지라서버에_붙일_프로젝트_리스트.add(지라프로젝트_엔티티_검색결과(지라프로젝트));
	...
	private JiraIssueStatusEntity 검색결과_미등록_이슈상태_엔티티_매핑(OnPremiseJiraStatusDTO 이슈상태) throws Exception {
	...
	private JiraIssueResolutionEntity 검색결과_미등록_이슈해결책_엔티티_매핑(OnPremiseJiraResolutionDTO 이슈해결책) throws Exception {
	...
	private JiraIssuePriorityEntity 검색결과_미등록_이슈우선순위_엔티티_매핑(OnPremiseJiraPriorityDTO 이슈우선순위) throws Exception {
	...
	private JiraIssueTypeEntity 검색결과_미등록_이슈타입_엔티티_매핑(OnPremiseJiraIssueTypeDto 이슈타입) throws Exception {
	...
	private JiraProjectEntity 검색결과_미등록_프로젝트_엔티티_매핑(OnPremiseJiraProjectDTO 지라프로젝트) throws Exception {
	...
	private JiraIssueStatusEntity 검색결과_미등록_이슈상태_엔티티_매핑(Status 이슈상태) throws Exception {
		JiraIssueStatusEntity 지라이슈상태_저장 = new JiraIssueStatusEntity();
		지라이슈상태_저장.setC_issue_status_id(이슈상태.getId());
		지라이슈상태_저장.setC_issue_status_name(이슈상태.getName());
		지라이슈상태_저장.setC_issue_status_url(이슈상태.getSelf());
	...
	private JiraIssueResolutionEntity 검색결과_미등록_이슈해결책_엔티티_매핑(Resolution 이슈해결책) throws Exception {
		JiraIssueResolutionEntity 지라이슈해결책_저장 = new JiraIssueResolutionEntity();
		지라이슈해결책_저장.setC_issue_resolution_id(이슈해결책.getId());
		지라이슈해결책_저장.setC_issue_resolution_name(이슈해결책.getName());
		지라이슈해결책_저장.setC_issue_resolution_url(이슈해결책.getSelf());
	...
	private JiraIssuePriorityEntity 검색결과_미등록_이슈우선순위_엔티티_매핑(Priority 이슈우선순위) throws Exception {
		JiraIssuePriorityEntity 지라이슈우선순위_저장 = new JiraIssuePriorityEntity();
		지라이슈우선순위_저장.setC_issue_priority_id(이슈우선순위.getId());
		지라이슈우선순위_저장.setC_issue_priority_name(이슈우선순위.getName());
		지라이슈우선순위_저장.setC_issue_priority_url(이슈우선순위.getSelf());
	...
	private JiraIssueTypeEntity 검색결과_미등록_이슈타입_엔티티_매핑(CloudJiraIssueTypeDTO 이슈타입) throws Exception {
		JiraIssueTypeEntity 지라이슈타입_저장 = new JiraIssueTypeEntity();
	...
	private JiraProjectEntity 검색결과_미등록_프로젝트_엔티티_매핑(CloudJiraProjectDTO 지라프로젝트) throws Exception {
		JiraProjectEntity 지라프로젝트_저장 = new JiraProjectEntity();
		지라프로젝트_저장.setC_jira_name(지라프로젝트.getName());
		지라프로젝트_저장.setC_jira_key(지라프로젝트.getKey());
		지라프로젝트_저장.setC_desc(지라프로젝트.getId());
	...
	private JiraProjectEntity 지라프로젝트_엔티티_검색결과(OnPremiseJiraProjectDTO 지라프로젝트) throws Exception {
		...
		return jiraProject.getNode(지라프로젝트_검색);
	}
	...
	private JiraIssueTypeEntity 지라_이슈타입_엔티티_검색결과(OnPremiseJiraIssueTypeDto 이슈타입) throws Exception {
		...
		return jiraIssueType.getNode(지라이슈타입_검색);
	}

	private JiraIssuePriorityEntity 지라_이슈우선순위_엔티티_검색결과(OnPremiseJiraPriorityDTO 이슈우선순위) throws Exception {
		JiraIssuePriorityEntity 지라이슈우선순위_검색 = new JiraIssuePriorityEntity();
		지라이슈우선순위_검색.setWhere("c_issue_priority_id", 이슈우선순위.getId());
		return jiraIssuePriority.getNode(지라이슈우선순위_검색);
	}

	private JiraIssueStatusEntity 지라_이슈상태_엔티티_검색결과(OnPremiseJiraStatusDTO 이슈상태) throws Exception {
		JiraIssueStatusEntity 지라이슈상태_검색 = new JiraIssueStatusEntity();
		지라이슈상태_검색.setWhere("c_issue_status_id", 이슈상태.getId());
		return jiraIssueStatus.getNode(지라이슈상태_검색);
	}

	private JiraIssueResolutionEntity 지라_이슈해결책_엔티티_검색결과(OnPremiseJiraResolutionDTO 이슈해결책) throws Exception {
		JiraIssueResolutionEntity 지라이슈해결책_검색 = new JiraIssueResolutionEntity();
		지라이슈해결책_검색.setWhere("c_issue_resolution_id", 이슈해결책.getId());
		return jiraIssuePriority.getNode(지라이슈해결책_검색);
	}

	private JiraIssueStatusEntity 지라_이슈상태_엔티티_검색결과(Status 이슈상태) throws Exception {
		JiraIssueStatusEntity 지라이슈상태_검색 = new JiraIssueStatusEntity();
		지라이슈상태_검색.setWhere("c_issue_status_id", 이슈상태.getId());
		return jiraIssueStatus.getNode(지라이슈상태_검색);
	}

	private JiraIssueResolutionEntity 지라_이슈해결책_엔티티_검색결과(Resolution 이슈해결책) throws Exception {
		JiraIssueResolutionEntity 지라이슈해결책_검색 = new JiraIssueResolutionEntity();
		지라이슈해결책_검색.setWhere("c_issue_resolution_id", 이슈해결책.getId());
		return jiraIssuePriority.getNode(지라이슈해결책_검색);
	}

	private JiraIssuePriorityEntity 지라_이슈우선순위_엔티티_검색결과(Priority 이슈우선순위) throws Exception {
		JiraIssuePriorityEntity 지라이슈우선순위_검색 = new JiraIssuePriorityEntity();
		지라이슈우선순위_검색.setWhere("c_issue_priority_id", 이슈우선순위.getId());
		return jiraIssuePriority.getNode(지라이슈우선순위_검색);
	}
